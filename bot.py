...
# ========== ФУНКЦИИ ДЛЯ РАБОТЫ С ПОЛЬЗОВАТЕЛЯМИ И СТАТУСАМИ ==========
...
def save_user(message: Message):
    ...
    try:
        ...
        if not exists:
            ...
            if referrer_id:
                # Добавляем реферала во внешнюю функцию
...
async def show_profile(message: Message):
    ...
    # Определяем статус
    if status_data['is_creator']:
        status_text = "👑 СОЗДАТЕЛЬ"
    elif status_data['is_admin']:
        status_text = "⚙️ АДМИН"
    elif status_data['is_vip']:
        status_text = "💎 VIP"
    else:
        status_text = "👤 ПОЛЬЗОВАТЕЛЬ"
    
    # Информация о лимитах
    if status_data['is_vip'] or status_data['is_admin']:
        limit_text = "∞ Безлимитно"
    else:
        limit_text = f"{status_data['downloads_left']}/1 на этой неделе"
    ...
    text = (
        ...
        f"**ID:** `{user_id}`\n"
        f"**Приглашений:** {status_data['invites']}\n"
        f"**Лимит скачиваний:** {limit_text}\n\n"
        f"**💎 Как получить VIP?**\n"
        f"Пригласи 1 друга в бота и получи VIP навсегда!\n\n"
        f"**Твоя реферальная ссылка:**\n"
        f"`{ref_link}`\n\n"
        f"Просто отправь эту ссылку друзьям. Когда они перейдут и запустят бота, ты получишь VIP."
    )
...
async def broadcast_photo(message: Message, state: FSMContext):
    ...
    for user_id in users:
        try:
            if photo_id:
                await bot.send_photo(chat_id=user_id, photo=photo_id, caption=text, parse_mode="Markdown")
            else:
